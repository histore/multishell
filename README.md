...
| ⌨️ **Comprehensive Keyboard Navigation** | First-class keyboard shortcuts for tabs (`Ctrl+Shift+T`, `Ctrl+W`, `Ctrl+Tab`), help (`F1`), history (`Ctrl+Shift+H`), and modals (`Esc`). |
...
│   ├── ITabStatePersistence.cs # Workspace JSON persistence contract
│   └── TabStatePersistence.cs  # Thread-safe atomic JSON file store
...
| `Escape` | Close active overlay / drawer / modal dialog and refocus terminal |

---

## 🧪 Testing

MultiShell includes a comprehensive xUnit test suite covering MVVM ViewModels, Process services, Localization, Dual-Theming, and Workspace persistence:
